---
title: تصميم Phase 3 — مكتبة المنتجات المشتركة ومحرك حل التوريد (Supply Resolution Engine)
status: DRAFT — بانتظار اعتماد المؤسس (لا تنفيذ قبل ذلك)
version: 0.1
last_updated: 2026-09-19
owner: Claude (تصميم) — بانتظار اعتماد نهائي من المؤسس
source_of_truth: هذا الملف لنطاق التصميم نفسه فقط — لا كود، لا Schema حي، لا Migration حتى تكليف
  صريح منفصل لاحقاً (بنفس نمط TASK-12 بعد اعتماد `PHASE_2_DOMAIN_DESIGN.md`)
related: specs/orders/PHASE_2_DOMAIN_DESIGN.md (النمط/الصرامة المرجعية)، docs/DATABASE.md §3/§3.1،
  ...
  docs/PRODUCT_ENGINE.md، docs/DECISIONS.md (ADR-022, ADR-031, ADR-032, DD-002, DD-003, DD-004)،
...
> **⚠️ تحذير تسمية إلزامي — اقرأ قبل أي شيء آخر.** اسم هذه الوثيقة (`PHASE_3_...`) **لا علاقة له**
> بترقيم "Phase 3" الموجود فعلاً في `specs/orders/REEF_V1_MASTER_EXECUTION_PLAN.md` (حيث Phase 3 =
> "تنفيذ Multi-Merchant"، **منجَزة بالفعل** — TASK-12/13، `docs/audits/2026-09-19-launch-readiness-
> report.md`). هذا تصادم تسمية حقيقي اكتُشف أثناء التحقق الحي لهذه المهمة بالذات، من نفس الفئة
> الموثَّقة سابقاً في المشروع (`world_scope` مقابل `data-world`، `worlds` مقابل `WorldSlug`، إسناد
> `ADR-025` الخاطئ في تعليقات كود CATALOG-IMPORT-WORKFLOW قبل تصحيحه بـ`ADR-031`). **القرار هنا:**
> يُبقى اسم الملف كما طلبه المؤسس حرفياً في موجّه هذه المهمة، لكن يُوثَّق هذا التصادم صراحة هنا وفي
> التقرير النهائي — لا يُفترَض أي تسلسل رسمي بين هذه الوثيقة وخطة `REEF_V1_MASTER_EXECUTION_PLAN.md`
> المرقَّمة. يُنصَح بإعادة تسمية هذا الملف (مثال: `SUPPLY_RESOLUTION_DESIGN.md` بلا بادئة رقم مرحلة)
> عند الاعتماد، بقرار المؤسس — غير مُنفَّذ هنا (تغيير اسم ملف خارج نطاق "توثيق فقط" الصارم لهذه المهمة).

> **Founder Gate — DRAFT، غير معتمَد.** لا كود، لا Migration، لا تعديل على أي ملف تطبيق. هذه الوثيقة
> **لا تحسم** أياً من الأسئلة المفتوحة في §24 — تُعرَض كأسئلة تحتاج قراراً صريحاً من المؤسس، تحديداً:
> العلاقة بين الحد الأدنى لهامش ريف وجدول العمولات القائم (`SALSABIL_CONSTITUTION.md §19`)، والقيمة
> الافتراضية العالمية للهامش. لا تُنفَّذ أي خطوة تالية (Migration، Task تنفيذ) دون اعتماد صريح منفصل.
...
3. **افتراضي عالمي** — **`OPEN_QUESTION` صراحة، لا يُحسَم هنا** (بطلب المؤسس المباشر في موجّه هذه المهمة،
   §24 بند 1). بلا قيمة مضبوطة فعلياً (لا قسم ولا استثناء)، **لا رفض صامت ولا قيمة مخترَعة** — سلوك
   `Fail Open` موثَّق صراحة (لا إنفاذ فعلي حتى يُضبَط، تماماً كنمط `BR-016` الحد الأدنى لقيمة الطلب:
   `OPEN_QUESTION` معلن، لا افتراض ضمني لرقم).
...
-- جدول جديد — الحد الأدنى لكل قسم من الهرمية الجديدة (catalog_categories، لا القديم)
...
- **وقت اعتماد/تعديل `catalog_master_items.base_price`** (`updateMasterItemPrice`، `[CODE]`، موجودة
  ...
  `products` مرتبط لا يخالف الحد الأدنى المُحسَم له (قسم/استثناء/لا حد بعد).
...
4. **الحد الأدنى لهامش ريف** — الهامش المحسوب (§3.2) `>= min_margin` الساري على قسم ذلك العنصر
   (أو الاستثناء الخاص بالمنتج)، **إلا إن كان لا حد مضبوط بعد (§3.2 بند 3) — عندها هذا القيد `Pass`
   تلقائياً (Fail Open موثَّق)**.
...
(§12، Schema فقط). **لا وزن افتراضي مقترَح هنا** — نفس منطق §3.2 بند 3 (لا قيمة مخترَعة بلا قرار مؤسس)،
الجدول يبدأ فارغاً، القيمة الفعلية تُضبَط عند التفعيل الحقيقي الأول.
...
`fulfillment_cost_estimates`/`merchant_delivery_offices`؛ المحرك نفسه (خدمة تطبيق تعمل بـ`service_role`)
هو الكاتب الوحيد على `resolution_records`.
...
بالكامل بلا مالك تجاري. **هذا التصميم لا يحل هذه المشكلة تلقائياً** — يفترضها كنقطة انطلاق (بمجرد أن
يُسند `tenant_id`/يُربَط `master_item_id` فعلياً لصفوف حقيقية، محرك حل التوريد يصبح ذا معنى عملي).

1. **لا `UPDATE` جماعي على `products.tenant_id` مباشرة** — هذا **قرار بيانات/عمل يخص المؤسس**
   (`docs/audits/2026-09-19-task-18-catalog-storefront-report.md §5`: "هل الكتالوج مباع مباشرة من
   ريف كمنصة، أم يُوزَّع على تجار حقيقيين لاحقاً؟") — **غير محسوم هنا، `OPEN_QUESTION` صريح (§24)**.
2. **المسار الآمن المقترَح إن اختار المؤسس "توزيع على تجار حقيقيين":** لكل تاجر جديد ينضم، يستورد Excel
...
- إنفاذ الحد الأدنى لهامش ريف على مستويين (قسم + استثناء منتج) — **بلا** قيمة افتراضية عالمية (`OPEN_QUESTION`).
...
| 13 | لا حد أدنى هامش مضبوط لا على القسم ولا كاستثناء منتج | القيد §6 بند 4 يمر (`Pass` تلقائي، Fail Open موثَّق) — لا رفض صامت لعرض تاجر بلا سبب واضح |
...
### 24.1 قرارات معمارية (مُتَّخَذة هنا، ضمن نطاق هذه الوثيقة نفسها فقط — بانتظار اعتماد المؤسس الشامل للوثيقة كلها)
...
### 24.4 أسئلة مفتوحة — تحتاج قراراً صريحاً من المؤسس (لا تُحسَم هنا)

1. **العلاقة بين الحد الأدنى لهامش ريف (§3) وجدول العمولات القائم في `SALSABIL_CONSTITUTION.md §19`**
   (نسب عمولة مختلفة لكل قسم: بقالة 5%، ألبان 25%، عطور 30%+...) — هل الحد الأدنى للهامش قيد **إضافي**
   فوق العمولة (تاجر يدفع عمولة % **و** يجب أن يحافظ على هامش مطلق أدنى)، أم الاثنان وجهان لنفس القيد
   يجب توحيدهما في مفهوم واحد؟ **لم يُحسَم هنا بطلب المؤسس الصريح.**
2. **القيمة الافتراضية العالمية للهامش الأدنى** (§3.2 بند 3) — بلا رقم، النظام يعمل بـ`Fail Open`
   (لا إنفاذ) حتى يُضبَط. **يحتاج رقماً صريحاً من المؤسس، أو قراراً بإبقائه `Fail Open` إلى أجل غير مسمى.**
3. **هل يُعالَج غياب `tenant_id` عن الـ7,506 منتجاً (§20) قبل أي تفعيل فعلي لهذا المحرك؟** — بلا هذا،
   المحرك جاهز تصميماً لكن بلا مادة عملية يعمل عليها (نفس ملاحظة تقرير TASK-18 §10 بند 4، لم يتغيّر).
4. **إعادة تسمية هذا الملف** (تحذير أعلى الوثيقة) — تجنّباً لالتباس "Phase 3" مع
   `REEF_V1_MASTER_EXECUTION_PLAN.md`.
5. **متى يُبنى `open_marketplace_v5` فعلياً** — مرتبط مباشرة بحسم `BR-017` (لا يزال `PROPOSED`، بلا
   قرار مؤسس نهائي) — غير محسوم هنا، بلا أي جدول زمني مقترَح.

### 24.5 امتدادات مستقبلية (أسماء محجوزة فقط، لا التزام)

`route_optimized_v2`، `capacity_aware_v3`، `ai_assisted_v4`، `open_marketplace_v5` (§9) — لوحة إدارة
لضبط `supply_resolution_weights`/`catalog_category_min_margins` (غير مبنية هنا، تحتاج واجهة `platform_
admin` جديدة) — حساب فعلي لـ`merchant_reliability_snapshots.reliability_score`/`fulfillment_cost_
estimates.estimated_cost` بقاعدة حقيقية بدل الإدخال اليدوي.

---

*نهاية الوثيقة — DRAFT بانتظار اعتماد المؤسس. راجع §0 لملخص التحقق الحي الكامل، و§24.4 لكل الأسئلة
التي تحتاج قراراً صريحاً قبل أي تنفيذ.*
