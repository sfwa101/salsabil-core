...
- إجراء تفعيل/تعطيل التاجر لا يُسجَّل في أي سجل تدقيق حالياً (لا `audit_log` عام بعد) — قرار مؤجَّل صراحة لليوم 12، لا سهو.
- `getRecentStatusHistory` بحد ثابت (50) بلا Pagination حقيقي — مقبول لحجم البيانات التجريبي الحالي.

## Open Questions

1. **متى تُستبدَل جلسة الهاتف المصغّرة لكل من التاجر والإدارة بمصادقة حقيقية؟** أولوية الإدارة أعلى (صلاحيات أوسع) — يحتاج قراراً صريحاً قبل أي بيانات إنتاجية حقيقية.
...
3. **متى يُبنى `audit_log` العام (اليوم 12) ليشمل إجراءات مثل تفعيل/تعطيل التاجر؟**

## Status

`PARTIALLY_IMPLEMENTED` — تسجيل دخول، إدارة تجار، رؤية/تحكم طلبات شاملة، وسجل تدقيق كلها `IMPLEMENTED` ومُختبَرة حياً (وحدة + تكامل + متصفح حقيقي بما فيه اختبارات أمنية سلبية). لا كلمة مرور/تحقق ثانٍ، لا `audit_log` عام يغطي الإجراءات الإدارية نفسها.
