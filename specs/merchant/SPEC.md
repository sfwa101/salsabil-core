...
3. **جديد (اليوم 10):** متى تُستبدَل جلسة الهاتف المصغّرة بمصادقة حقيقية (كلمة مرور/OTP/Supabase Auth)؟ يحدد هذا مباشرة متى يصبح تسجيل تاجر ثانٍ حقيقي آمناً.
4. **جديد:** هل تحتاج `merchant_manager`/`employee` نطاقاً فعلياً قريباً، أم يبقى مالك واحد لكل تاجر لفترة أطول؟

## Status

`PARTIALLY_IMPLEMENTED` — عزل بيانات فعلي ومُختبَر (قراءة وكتابة)، تسجيل دخول حقيقي وبوابة طلبات تعملان حياً، لا تسجيل تاجر جديد عبر واجهة، لا مصادقة قوية (كلمة مرور/OTP) بعد.
