...
// اليوم" — هذا غير دقيق. `SelectableOptionType` الفعلي في src/core/modules/catalog/types.ts (rename
// من `ProductOptionType`، راجع docs/DECISIONS.md) يحتوي حصراً 'size' | 'addon' — لا نوع 'prepMethod'
// ولا 'packaging' في المخطط أو في CatalogService (validateSelection/calculatePrice) اليوم. إضافة نوع
// جديد لـProductOption تمس قاعدة عمل حقيقية (التسعير/التحقق من الاختيار، ADR-004) — Guardian Matrix
// DEEP وفق docs/AGENTS.md §17 (Financial logic)، تحتاج قراراً مؤسس صريحاً منفصلاً، لا تُخترَع هنا
// كجزء من دفعة بصرية. البلوكان مُسجَّلان أدناه بمعرّف/ترتيب/تسمية جاهزين (البنية موجودة كما طُلب)،
// لكن `isVisible` لكل منهما يعيد `false` دائماً — لا مستهلك بيانات حقيقي حتى تُضاف الأنواع فعلياً
// لـSelectableOptionType (قرار مستقل مستقبلي).

import type { Product, SelectableOptionType } from '@/core/modules/catalog/types';
...
function hasOptionType(product: Product, type: SelectableOptionType): boolean {
...
export const PRODUCT_PAGE_BLOCKS_REGISTRY: ProductPageBlockDefinition[] = [
  ...
  // CONCEPTUAL — راجع تحذير أعلى الملف. لا 'prepMethod' في SelectableOptionType اليوم.
  ...
  // CONCEPTUAL — راجع تحذير أعلى الملف. لا 'packaging' في SelectableOptionType اليوم.
