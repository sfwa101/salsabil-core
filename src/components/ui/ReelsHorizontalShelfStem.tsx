...
export interface ReelsHorizontalShelfStemProps {
  items: ReelSnapshot[];
  onReelClick: (reel: ReelSnapshot) => void;
}
    ...
    <div className="w-full flex flex-col gap-3 py-2" dir="rtl">
      ...
      <div className="flex items-center justify-between px-2 sm:px-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base sm:text-lg font-bold text-foreground">ريلز ووصفات ريف المدينة</h2>
        </div>
        ...
        <div className="flex gap-3 sm:gap-4 w-max">
          {items.map((reel) => (
            <button 
              ...
              className="w-32 h-56 sm:w-44 sm:h-72 shrink-0 rounded-2xl relative overflow-hidden group cursor-pointer shadow-sm border border-border/40 snap-start text-right"
            >
              ...
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-scrim/80 via-scrim/20 to-transparent"></div>
              
              {/* Platform Badge */}
              <div className="absolute top-2 left-2 bg-scrim/40 backdrop-blur-md text-scrim-foreground rounded-full p-1.5 shadow-sm border border-scrim-foreground/10">
                {getPlatformIcon(reel.platform)}
              </div>

              {/* Play Button */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-scrim-foreground/30 backdrop-blur-md flex items-center justify-center text-scrim-foreground border border-scrim-foreground/50 group-hover:bg-scrim-foreground/50 transition-colors">
              ...
              <div className="absolute bottom-0 inset-x-0 p-3 flex flex-col gap-1">
                <span className="text-scrim-foreground text-xs sm:text-sm font-bold line-clamp-2 leading-tight drop-shadow-md">
                  {reel.title}
                </span>
                <span className="text-scrim-foreground/80 text-[10px] font-medium drop-shadow-sm">
                  {reel.chefOrSource}
                </span>
              </div>
            </button>
