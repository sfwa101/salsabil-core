...
import { calculatePriceAction } from '@/app/product/[id]/actions';
  ...
  return (
    <div className="flex flex-col gap-6">
      {sizeOptions.length > 0 && (
        <div>
          <h2 className="mb-2 text-sm font-medium text-stone-600">الحجم</h2>
          <div className="flex flex-col gap-2">
            {sizeOptions.map((option) => (
              <label
                key={option.id}
                className={`flex cursor-pointer items-center justify-between rounded-xl border p-3 ${
                  sizeId === option.id ? 'border-brand-green bg-brand-green/5' : 'border-stone-200'
                }`}
              >
      ...
      {addonOptions.length > 0 && (
        <div>
          <h2 className="mb-2 text-sm font-medium text-stone-600">إضافات</h2>
          <div className="flex flex-col gap-2">
            {addonOptions.map((option) => (
              <label
                key={option.id}
                className={`flex cursor-pointer items-center justify-between rounded-xl border p-3 ${
                  addonIds.includes(option.id) ? 'border-brand-green bg-brand-green/5' : 'border-stone-200'
                }`}
              >
                <span className="flex items-center gap-2">
