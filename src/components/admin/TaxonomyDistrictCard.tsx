'use client';
// §31 بند 8 — حي واحد كامل: تعديل (اسم/ترتيب/تفعيل)، قائمة أقسامه الرئيسية (كل واحد مع أقسامه
// الفرعية عبر TaxonomyCategorySection)، ونموذج إضافة قسم رئيسي جديد.

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { updateDistrictAction, createCategoryAction } from '@/app/admin/taxonomy/actions';
...
export function TaxonomyDistrictCard({ district, categories }: TaxonomyDistrictCardProps) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setError(null);
    const result = await updateDistrictAction({ id: district.id, nameAr, sortOrder: Number(sortOrder), isActive });
    if ('error' in result) {
      setStatus('error');
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-2xl border p-4 ${isActive ? 'border-border bg-card' : 'border-border/50 bg-muted/30 opacity-70'}`}>
      <form onSubmit={handleSave} className="flex flex-wrap items-center gap-2">
        <span dir="ltr" className="font-mono text-xs text-muted-foreground">{district.slug}</span>
      ...
      <div className="flex flex-col gap-2 ps-3">
        {categories.map(({ category, subcategories }) => (
          <TaxonomyCategorySection key={category.id} category={category} subcategories={subcategories} />
        ))}
