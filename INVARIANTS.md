...
⚠️ تحديث دليل جزئي (2026-09-24، P0-GUEST-CART-CHECKOUT-REGRESSION، لا استبدال): الجزء الأول من هذا
المسار (زائر جديد بلا كوكيز ← فتح منتج حقيقي ← إضافة للسلة ← `/cart` يعرض البند ← `/checkout` يعرض
البند) **نُفِّذ حياً ضد staging.reefam.com فعلياً** عبر Playwright (سياق متصفح جديد كل مرة، 4/4 نجاح،
صفر أخطاء console) — إغلاق `DECISION-DEBT-004` (راجع الإدخال أعلاه). التوقف كان **متعمَّداً قبل** تعبئة
بيانات التوصيل وإرسال الطلب (بند 8 في السكربت الكامل) بتوجيه صريح من نطاق تلك المهمة نفسها — لا طلب
حقيقي جديد نتج عن هذا التحقُّق. الفجوة الأصلية (لا تشغيل كامل، بما فيه بند 8-10، ضد staging تحديداً)
**لا تزال قائمة جزئياً** — فقط النصف الأول (حتى Checkout) مُغطى حياً على staging الآن.
```
