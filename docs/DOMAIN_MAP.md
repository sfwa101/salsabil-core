...
| يستخدمه | كل التطبيقات الثلاثة (Customer, Merchant, Admin)؛ `listActiveWorlds()` أيضاً عبر `src/components/WorldSwitcher.tsx` (اليوم 29، BAYAN-HOME-FEED-001) — أول مستهلك واجهة عميل مباشر لها |
...
| يستخدمه | ريف (الخلاصة الرئيسية، BAYAN-HOME-FEED-001 — لوحة الإدارة `DONE` اليوم 24، ترويسة العميل `DONE` اليوم 26، الخلاصة الفعلية اليوم 27) |
| الحالة | `PARTIALLY_IMPLEMENTED` — الباك-إند (اليوم 23) كامل ومُتحقَّق حياً (8/8، `ADR-021`). لوحة الإدارة (اليوم 24، `src/app/admin/posts/`) كاملة ومُتحقَّقة حياً (14/14). **واجهة العميل جزئية اليوم 26:** ترويسة (`FeedTopBar`/`StoryBar`/`FeedTabBar`) فوق محتوى الأحياء القائم بلا استبداله — **لا خلاصة منشورات فعلية بعد** (لا استدعاء لـ`bayanService.listFeed()` من أي صفحة عميل حتى الآن، `FeedTabBar` يهيّئ الحالة `?tab=` فقط بانتظار استهلاكها). الهوية البصرية (`slug: bayan`، `docs/UI_UX_SYSTEM.md §8.4`) لا تزال غير مُستخدَمة في صفحة الخلاصة (تبقى `data-world="reef"` — راجع تحذير `docs/DATABASE.md §4`). **لا يحسم `OPEN_QUESTION-001` أعلاه** — بناء واجهة بيان لا يقرر تلقائياً هل خليل/حكيم/برق/تيسير الأربعة الباقون "عوالم" أم "محركات نواة" |
