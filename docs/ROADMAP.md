...
>
> **⚠️ قيد معروف موحَّد (لا تجاهل صامت) — قفلان في-الذاكرة من نفس الفئة:** `src/core/kernel/security/rate-limit.ts` (Rate Limiting على الدخول، `ADR-014`) و`inFlightCheckouts` الجديد (Idempotency على Checkout، `ADR-022`) كلاهما `Map` عملية واحدة — لا ينجوان من إعادة تشغيل الخادم أو تعدد النسخ (Serverless/عدة خوادم). مقبولان مؤقتاً لمرحلة خادم واحد/حجم تجريبي، **يجب** إعادة تقييمهما معاً (قفل موزَّع/Redis) ببند واحد قبل أي نشر متعدد الخوادم — لا بندين منفصلين.
