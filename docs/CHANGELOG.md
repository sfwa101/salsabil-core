...
- **`src/app/test-integration/page.tsx`** — حارس خادم صريح (`if (process.env.NODE_ENV === 'production') notFound();`)
  في أعلى الـServer Component. يمنع الوصول لمسار POC الوحيد المُثبَت أنه يكتب فعلياً على `cart_items`
  الحقيقية (`/test-integration`، راجع docs/audits/2026-09-21-real-backend-sdui-integration-poc.md
  وFinding 2 في docs/audits/2026-09-22-post-antigravity-integration-forensic-audit.md §19) في أي بيئة
  مبنية (`next build` يضبط `NODE_ENV=production` على Vercel production وpreview/staging معاً)، ويُبقيه
  متاحاً فقط تحت `next dev` المحلي. `/test-ui`و`/test-portability` (مسارا mock فقط، لا كتابة حقيقية)
  **لم يُمسّا عمداً** — خارج نطاق هذه المهمة.
- **`docs/DECISIONS.md`** (v1.40 → v1.41) —
  - `DD-022`: إضافة "SEC-P1-1 Closure Detail (2026-09-22)" — يُغلق تسريب `inventory.cost_price` نفسه
    (`CLOSED`)، مُفرِّقاً صراحة بين التحقق التطبيقي (مكتمل، كود مُتحقَّق منه في هذه الجلسة) والتحقق
    الحي لقاعدة البيانات (**FOUNDER EXTERNAL VERIFICATION** — تحقُّق مباشر من المؤسس خارج هذه الجلسة،
    لا استعلاماً نفَّذه Claude Code). قرار الفصل الكامل لـ`cost_price` إلى جدول/عرض منفصل يبقى `OPEN`
    بشكل مستقل، كما كان. استشهاد `SALSABIL_BACKEND_ARCHITECTURE_FORENSIC_AUDIT` المكسور في السطر
    القديم مُعلَّق عليه صراحة، لا مُصلَح (تسجيل فقط).
  - `ADR-035` (جديد، `ACCEPTED`): "SDUI / Stem كطبقة عرض عبر Adapters" — يُرسِّخ رسمياً نمط التكامل
    القائم فعلياً (يُحيل لـdocs/salsabil-frontend-integration-pattern.md بدل تكراره)، ويضيف أربع قواعد
    لم تكن موثَّقة صراحة: Stem طبقة عرض لا بديل نطاق، منع بنية إنتاج وهمية موازية، قاعدة خط تجميع
    لـAntigravity، واعتماد POC 2026-09-21 كنمط مرجعي رسمي. يُغلق Finding 4 (فجوة ADR/DD مفقودة) من
    docs/audits/2026-09-22-post-antigravity-integration-forensic-audit.md §19.
- **لا تعديل على Schema أو منطق أعمال أو صلاحيات في هذه المهمة** — حارس مسار واحد (سطر واحد فعلياً) +
  توثيق/حوكمة فقط. لم يُنفَّذ SQL، لم تُستهلَك أي صلاحية L4+ (راجع AGENTS.md §14).

---

## 2026-09-20 — ERP-SAAS-VISION-AND-PRICING-MODE: توثيق فقط، بلا كود ولا Schema

- **`docs/ERP_SAAS_VISION.md`** (جديد) — رؤية سلسبيل بعيدة المدى كمنصة SaaS/ERP شاملة قابلة
  للاستخدام من أي متجر/شركة/مصنع (مخزون، تسعير، كاشير/POS، سوشيال ميديا، عملاء، تسويق، دورات
  تعليمية)، بتكليف مباشر من المؤسس. ريف المدينة موثَّق كأول تطبيق فعلي لمحركات هذه المنصة، لا نظام
  منفصل عنها. يوثّق مبدأ البناء بمراحل متلاحقة، والقاعدة الحاكمة لتقييم كل قرار معماري حالي في ضوء
  هذه الرؤية (هل يمنعها مستقبلاً أم يمهّد لها).
- **`SALSABIL_CONSTITUTION.md`** (v1.5 → v1.6) — إضافة مرجعية فقط: سطر في §0 وفقرة في نهاية §5
  تشيران إلى `docs/ERP_SAAS_VISION.md` كمرجع أساسي يُقرأ أول أي مهمة مستقبلية تخص لوحات
  الإدارة/التاجر. بتكليف مؤسس مباشر (استثناء `docs/DOCUMENTATION_RULES.md §5.1`، سجل مستقل: `DD-021`).
- **`docs/ARCHITECTURE.md`** (v1.11 → v1.12) — بند جديد في §9 ("ما لا يُبنى الآن") يوثّق أن منصة
  SaaS/ERP العامة رؤية بعيدة المدى غير مبنية اليوم، مع إحالة لـ`docs/ERP_SAAS_VISION.md`.
- **`docs/DECISIONS.md`** (v1.38 → v1.39) — `ADR-034` (قرار معلَّق، `PROPOSED`): تصميم مقترح لنمط
  تسعير مختلط لكل منتج (`pricing_mode`) على مستوى الفئة (افتراضي) والمنتج الفردي (تجاوز اختياري) —
  توثيق تصميم فقط، بلا DDL ولا تعديل كود. يوثّق صراحة أن محرك المطابقة العادلة
  (`SALSABIL_CONSTITUTION.md §29.3`، `BR-005`) هو المرجع الحاكم الكافي لحالة "نفس المنتج بأسعار
  مختلفة عند عدة تجار" — لا حاجة لقرار جديد هناك. كذلك `DD-021` (توثيق واقعة تكليف التعديل على
  الدستور، RESOLVED فور كتابته).
- **لا تعديل على أي Schema أو كود تطبيقي في هذه المهمة** — توثيق/تصميم فقط بالكامل.

---

## 2026-09-07 — FIX-DD-010-CART-QUANTITY-UI-STALE: تصحيح تشخيص + إصلاح فعلي

> **⚠️ تصحيح تشخيص سابق:** `DD-010` (سُجِّل أثناء REBUILD-CART-CHECKOUT-FROM-LOVABLE-REFERENCE)
> افترض خلل Caching خاص بوضع الإنتاج (`next start`) تحديداً. تحقُّق حي دقيق (قياس الزمن الفعلي عبر
> استقصاء DOM كل 50-100ms) أثبت **لا فرق بين `next dev` وَ`next start`** — كلاهما يعمل، فقط بعد
> ~1000-1400ms، لا فوراً. الفشل الظاهري الأصلي كان انتظاراً قصيراً جداً (600ms) في اختبار التشخيص
> الأول، لا خللاً بيئياً حقيقياً. راجع `DD-010` المُحدَّث في `docs/DECISIONS.md` للتفصيل الكامل.

- **`src/components/CartActionButton.tsx`** (جديد) — غلاف عميل حول أزرار +/-/حذف في السلة يستخدم
  `useFormStatus()` (نمط قياسي لـServer Actions) لعرض مؤشر Pending فوري (أيقونة دوّارة + تعطيل)
  عند النقر — يحل مشكلة "تبدو الواجهة متجمّدة" بصرف النظر عن مدة الاستجابة الفعلية.
- **`src/app/(reef)/cart/page.tsx`** — تجميع التاجر ورف "غالباً ما يُشترى معه" (مستقلان تماماً عن
  بعضهما) كانا يُنتظَران بالتتابع رغم ذلك — `Promise.all` يدمجهما، يقلّل الزمن الكلي فعلياً لا
  إدراكياً فقط.
- **تحقُّق حي تحت `npm run build && npm start` تحديداً (لا `npm run dev`، بطلب صريح):** الزر
  يُعطَّل ويعرض مؤشراً دوّاراً خلال ~150ms من النقر (تحقَّقتُ فعلياً عبر لقطة شاشة)، لكل من +/-،
  صفر أخطاء Console. `tsc --noEmit` نظيف، `arch:check` نظيف (143 وحدة، 441 اعتماد)، 127/127 اختبار
  وحدة.
- **توثيق:** `docs/DECISIONS.md` (`DD-010` → `RESOLVED`، تشخيص مُصحَّح صراحة لا مُستبدَل صامتاً).
