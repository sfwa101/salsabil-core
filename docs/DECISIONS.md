...
## ADR-034
```
Title: نمط التسعير المختلط لكل منتج (pricing_mode) — تصميم مقترح، بلا DDL ولا تعديل كود
Status: ACCEPTED — اعتمده المؤسس مباشرة (2026-09-20). توثيق تصميم فقط حتى الآن — لا Migration/
          تعديل فعلي على `products`/`categories` بعد؛ التنفيذ يُكلَّف لاحقاً بمهمة منفصلة.
Date: 2026-09-20
Context: وصف المؤسس مباشرة نمط تسعير مختلط قائم فعلياً في الواقع التجاري: بعض المنتجات (بقالة أساسية
          غالباً) سعرها ثابت يحدده ريف/سلسبيل نفسها. بعضها الآخر (ملابس/عطور/هدايا/أحذية، وبعض
          البقالة مؤقتاً) سعرها يحدده التاجر نفسه — سعر بيع (Selling Price) + سعر توريد/تكلفة
          (Supply/Cost Price). التصميم الحالي (`docs/DATABASE.md → products.base_price`) عمود سعر
          واحد بلا تمييز "مَن يحدد السعر" — لا يعبّر عن هذا التمايز اليوم.
Decision: إضافة حقل منطقي `pricing_mode` على مستويين (تصميم فقط، بلا Migration فعلية):
          (أ) **مستوى الفئة** (جدول `categories` الحالي — وهو ما يقصده المؤسس مفهومياً بـ"catalog_
          categories"؛ لا جدول منفصل بهذا الاسم موجود اليوم، راجع docs/DATABASE.md): قيمة افتراضية
          لكل الفئة، بقيمتين مقترحتين: `platform_fixed` (سعر يحدده ريف/سلسبيل) أو `merchant_set`
          (سعر يحدده التاجر: بيع + توريد).
          (ب) **مستوى المنتج الفردي** (جدول `products`): عمود اختياري (`nullable`) يتجاوز افتراضي
          فئته لمنتج بعينه — عند `null` يُستخدَم افتراضي الفئة؛ عند وجود قيمة، تتجاوزه. هذا يغطي
          الاستثناء الذي وصفه المؤسس صراحة ("بعض البقالة مؤقتاً" رغم أن أساس فئة البقالة
          `platform_fixed`).
          هذا النمط (توريث افتراضي + تجاوز اختياري فردي) **ليس جديداً في هذا المستودع** — نفس فكرة
          `ADR-007` (Semantic Tokens + `theme_overrides` لكل مستأجر لاحقاً)، مطبَّقة هنا على التسعير
          بدل الألوان.
Alternatives: (أ) عمود واحد فقط على مستوى المنتج بلا افتراضي على مستوى الفئة — يتطلب ضبطاً يدوياً
          لكل منتج جديد، عبء تشغيلي كبير مع آلاف المنتجات الحالية (راجع DD-020: 99.3% من الكتالوج
          بلا حتى `tenant_id` فعلي اليوم).
          (ب) عمود واحد فقط على مستوى الفئة بلا تجاوز فردي — لا يغطي حالة "بعض البقالة" التي وصفها
          المؤسس صراحة كاستثناء داخل فئة أساسها `platform_fixed`.
          (ج) توسيع `products.options` (JSONB، `ADR-004`) بدل عمود مخصَّص — مرفوض هنا: `pricing_mode`
          قيمة بنيوية تحكم منطق حساب السعر نفسه وتحتاج فهرسة/استعلاماً مباشراً، بعكس `options` الذي
          صُمِّم أصلاً لخيارات عرضية للمنتج (حجم/إضافة) لا لمنطق تسعير بنيوي.
Why: التوريث + التجاوز الاختياري يوازن بين تقليل العبء التشغيلي (لا ضبط يدوي لكل منتج) والمرونة
          الفعلية (استثناءات فردية موصوفة صراحة من المؤسس) — نفس المنطق المعتمَد سابقاً في `ADR-007`
          لمشكلة مشابهة بنيوياً (توريث عام + تجاوز محلي اختياري).
Consequences (عند التنفيذ الفعلي لاحقاً): يتطلب حسم القيم الدقيقة المسموحة لـ`pricing_mode` (enum ثابت
          أم نص حر؟) وحسم صلاحيات التعديل لكل مستوى (الإدارة لمستوى الفئة، التاجر لتجاوزه الفردي ضمن
          الفئات المسموح له فيها؟) — كلاهما `OPEN_QUESTION` لا يُحسَم في هذا القرار، يُحسَم عند
          التنفيذ الفعلي. يتقاطع مع Supply Resolution Engine القائم تصميمه فعلاً
          (`specs/orders/SUPPLY_RESOLUTION_ENGINE_DESIGN.md`, `DD-020`) عندما يبيع أكثر من تاجر نفس
          الصنف تحت `merchant_set` بأسعار مختلفة — هذا التقاطع محكوم بالفعل بمحرك المطابقة العادلة
          (راجع البند التالي)، لا يحتاج حسماً إضافياً هنا. **لا تعديل فعلي على `products`/`categories`
          في هذا القرار** — تصميم مقترح فقط بانتظار اعتماد المؤسس، ثم Migration منفصلة عند التنفيذ.

**توضيح صريح مطلوب (بند إضافي بطلب المؤسس، لا حاجة لقرار جديد):** حالة "نفس المنتج بأسعار مختلفة عند
          عدة تجار" (حين `pricing_mode = merchant_set`) **محكومة بالفعل بالكامل** بمحرك المطابقة
          العادلة المعتمَد في `SALSABIL_CONSTITUTION.md §29.3` (و`BR-005` في `docs/BUSINESS_RULES.md`):
          للسلع الأساسية، ارتفاع سعر ملحوظ عن متوسط المنطقة → علم "يحتاج تحقيقاً" تلقائي؛ للسلع
          الرفاهية، الأرخص يظهر افتراضياً؛ عند تساوي الأسعار، توزيع الطلبات بالدور. **لا يحتاج هذا
          القرار (`pricing_mode`) أي منطق عدالة/مطابقة إضافي منفصل** — هو فقط يحدد "من يضبط السعر
          ابتداءً"، بينما §29.3/`BR-005` يحدد "كيف تُعرَض/تُوزَّع الأسعار المتعددة الناتجة" — طبقتان
          متكاملتان لا تعارض بينهما ولا ازدواج.
Status: ACCEPTED — اعتمده المؤسس مباشرة (2026-09-20، راجع أعلى القرار). التوضيح أعلاه (حالة تعدد
          التجار) لا يضيف قراراً معلَّقاً خاصاً به — القيم الدقيقة المسموحة لـ`pricing_mode` وصلاحيات
          التعديل لكل مستوى تُحسَم عند التنفيذ الفعلي (راجع Consequences أعلاه)، لا تُبقي القرار نفسه
          معلَّقاً. **لا تنفيذ (DDL أو كود) قبل ذلك الحسم.**
Related Documents: docs/DATABASE.md (`products`, `categories`)، ADR-004 (JSONB مرن — لماذا
          `pricing_mode` ليس داخله)، ADR-007 (نمط التوريث+التجاوز المطابق)، DD-020 (Supply Resolution
          Engine، تعدد الموردين لنفس الصنف)، SALSABIL_CONSTITUTION.md §29.3 (محرك المطابقة العادلة)،
          docs/BUSINESS_RULES.md → BR-005، docs/ERP_SAAS_VISION.md (السياق الأوسع: محركات تسعير
...
### CONFLICT-010
```
بين: docs/audits/REEF_PHASE_1_PRODUCT_COMPLETENESS_AUDIT.md (2026-09-19، §17 مقدمة الملف، §25،
          §28، §29 بند 21 — "الصفحة الرئيسية لا تعرض أياً من الكتالوج الحقيقي...فقط 19 منتجاً
          تجريبياً وهمياً") مقابل docs/audits/2026-09-19-fix-fake-products-darkmode-execution-report-staging.md
          (نفس اليوم 2026-09-19، سابق زمنياً — يوثّق تنفيذ scripts/seed-real-merchant-highlight-posts.ts
          --env=staging وتحقّقاً حياً بـcurl أن الرئيسية تعرض بالفعل منتجات حقيقية، صفر poultry-test).
الوصف: أثناء تنفيذ §31 بند 3 (AUTONOMOUS-OVERNIGHT-BUILD-PHASE1-BLOCKERS، 2026-09-20)، تحقَّقت مباشرة
          من قاعدة بيانات staging وcurl حي ضد staging.reefam.com **قبل كتابة أي كود** — النتيجة تطابق
          تقرير التنفيذ (20 منشوراً حقيقياً منشوراً لمنتجات pilot-merchant، 0/19 منشوراً وهمياً لا يزال
          منشوراً، صفر إشارة poultry-test/demo-products في HTML الحي) لا ادعاء تقرير التدقيق. تقرير
          التدقيق نفسه (REEF_PHASE_1...) يذكر في مقدمته أنه "بُني من خمسة تقارير بحث مستقلة...حيثما
          وُجد تعارض...رجّحت هذا التقرير حالة الكود المتحقَّق منها مباشرة الآن" — لكن يبدو أن تقارير
          البحث الخمسة المُجمَّعة لم تكن على علم بتقرير تنفيذ fix-fake-products-darkmode من نفس اليوم
          (تسلسل زمني دقيق غير محسوم من الوثيقتين وحدهما)، فبقي الادعاء القديم في النسخة النهائية.
التأثير: §29 بند 21 وأي إحصاء إجمالي في §31 يعتمد عليه (الأرقام الختامية، Phase 1 Blockers count)
          يحمل معلومة غير مطابقة لحالة الكود الحية اليوم بخصوص "منتجات وهمية على الرئيسية" تحديداً —
          الفجوة المعمارية الأعمق التي يطلبها §31 بند 3 (الاعتماد الحصري على منشورات بيان بدل ربط
          مباشر بكامل الكتالوج القابل للشراء) **تبقى قائمة فعلياً** رغم زوال العرض الوهمي — لذا لم
          يُسقَط تنفيذ بند 3 بالكامل، نُفِّذ بنطاق مُعدَّل (راجع سجل البناء الليلي 2026-09-20، بند 3).
يحتاج قراراً من: المؤسس — تحديث §29 بند 21/الأرقام الختامية في REEF_PHASE_1_PRODUCT_COMPLETENESS_AUDIT.md
...
### DD-024
```
Decision: (1) Reels/Post Types — تعريف رسمي لأنواع منشور Bayan الستة، يحل محل الاستخدام الغامض السابق
          لـpost_type='reel' (صورة): (أ) reel — فيديو مستورَد برابط خارجي (YouTube/TikTok/Instagram/…)؛
          الرفع المباشر ميزة مستقبلية منفصلة، غير مبنية الآن. (ب) صورة مفردة. (ج) معرض صور. (د) مقالة
          بلا منتج. (هـ) مقالة + منتج واحد مرتبط. (و) مقالة + مجموعة منتجات مرتبطة. "reel" من الآن
          يعني حصراً الفيديو المستورَد بالرابط الخارجي المعرَّف هنا.
          (2) Fast-Track Execution Mode — وضع تنفيذ مؤقت مرتبط بضغط الوقت: الوكيل يستنتج نية المنتج من
          بنية الـStem + سياق المشروع القائم ويبني الـBackend الحقيقي اللازم دون التوقف لطلب قرار مؤسس
          عند كل عنصر backlog — يسجِّل كل قرار جوهري كإدخال DD منفصل بدل التوقف. الاستثناءات بلا تغيير:
          تعرُّض أمني حقيقي، منطق دفع يتجاوز الدفع عند الاستلام، عمليات لا رجعة فيها/مدمِّرة، وغموض
          حقيقي بلا أي إشارة توجيهية (zero-signal ambiguity) — كل هذه توجب التوقف والسؤال رغم الوضع. لا
          بيانات مفبركة، لا تجاوز أمني، لا تعديل على ملفات SDUI الجوهرية
          (DataResolver/PageEngine/ApplicationRuntime/ActionRouter/CapabilityRegistry) — بصرف النظر عن
          هذا الوضع.
Reason: (1) Reels: مهمة "Fast-Track Execution" تطلَّبت وصل ReelsHorizontalShelfStem/ReelsEmbedModalStem
          ببيانات حقيقية، وعقد Bayan الحالي يفتقر تعريفاً رسمياً موحَّداً لأشكال المنشور الستة يحدد ماذا
          يعني "reel" تحديداً (فيديو بالرابط) مقابل الاستخدام السابق الغامض لنفس القيمة كصورة.
          (2) Fast-Track: ضغط زمني ومحدودية استهلاك Claude Code الفعلي. نص مهمة "Fast-Track Execution"
          (2026-09-22) استشهد بهذا الإدخال (DD-024) كإذن مسبق قبل أن يكون موجوداً فعلياً في الملف —
          فجوة توثيقية حقيقية اكتُشفت وأُبلِغت صراحة كـCONFLICT وفق §6 قبل أي تنفيذ. المؤسس أكَّد
          التفعيل مرتين: أولاً عبر رد مباشر على سؤال توضيحي (AskUserQuestion) بعد اكتشاف الفجوة، ثم عبر
          إرسال نص هذا الإدخال بالضبط (2026-09-23) ليحل محل المسودة الأولية التي كتبها الوكيل استناداً
          لذلك الرد.
Risk: يوسِّع صلاحية الوكيل الذاتية (AI Agent Authority — DEEP-tier وفق §17) بإسقاط اشتراط "توقف وانتظر
          قرار مؤسس منفصل" لكل Stem يفتقر backend، ضمن الاستثناءات المذكورة أعلاه فقط. لا يُسقِط أي حماية
          أخرى: أي عنصر backlog يمس نطاقاً DEEP فعلياً (Schema حساس، أمان/RLS، منطق مالي، Tenant
          Isolation) يبقى يستوجب مراجعة Guardian مستقلة الشدة المناسبة كالمعتاد. تعريف Reels الجديد
          يستبدل دلالة post_type='reel' القديمة — أي كود/استعلام يعتمد على الدلالة القديمة (صورة) يحتاج
          مراجعة عند تطبيق هذا الإدخال (يُفحَص في Phase 0 الخاص بعنصر Reels نفسه).
Owner: Founder
Created: 2026-09-22 (مسودة أولية بعد تأكيد شفهي)، 2026-09-23 (نص نهائي مُرسَل من المؤسس يحل محل المسودة)
Review by: عند انتهاء Backlog المذكور في مهمة "Fast-Track Execution"، أو أي توسيع لاحق لنطاق هذا الإذن
Blocking: NO — لا يمنع أي عمل حالي؛ يُغيِّر آلية الموافقة لعناصر backlog محددة، ويعرِّف دلالة Reels
Status: ACTIVE
