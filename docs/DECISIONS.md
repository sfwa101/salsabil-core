...
### DD-019
```
Decision: هل يُعتمَد تصميم specs/orders/PHASE_3_SUPPLY_RESOLUTION_DESIGN.md — مكتبة منتجات مشتركة
          موسَّعة فوق ADR-031 القائم، محرك حل توريد جديد (Supply Resolution Engine)، إنفاذ حد أدنى
          لهامش ريف (قسم + استثناء منتج)، تدفق مراجعة مزدوج (Full/Lightweight Review) مع مطابقة
          بالباركود أولاً، وجدول catalog_content_review_queue منفصل — كنطاق تنفيذ رسمي قادم (Task
          تنفيذ منفصلة تماماً، بنفس نمط TASK-12 بعد اعتماد PHASE_2_DOMAIN_DESIGN.md)؟
Reason: DESIGN-TASK-01 (مهمة توثيق/تصميم بحتة، صفر كود/SQL تنفيذي، صفر تعديل على Checkout/Orders/Cart)
          ...
          بالفعل).
Risk: لا خطر تشغيلي فوري — هذا سجل تصميم `DRAFT`، لا كود منفَّذ ولا Migration مُطبَّقة على أي بيئة. الخطر
          المتبقي لو اعتُمد هذا التصميم لاحقاً بلا حسم الأسئلة المفتوحة المسجَّلة داخل الوثيقة نفسها
          (§24.4): (أ) إنفاذ حد أدنى للهامش بلا علاقة محسومة بجدول العمولات (`SALSABIL_CONSTITUTION.md
          §19`) قد يُنتج قيداً مزدوجاً غير مقصود على التاجر (عمولة % + هامش مطلق معاً بلا توحيد مفاهيمي)؛
          (ب) محرك حل التوريد جاهز تصميماً بلا مادة عملية يعمل عليها طالما 7,506 من 7,556 منتجاً (99.3%)
          بلا `tenant_id` فعلي (`docs/audits/2026-09-19-task-18-catalog-storefront-report.md §5`)؛
          (ج) `catalog_master_items.category_id` (قديم) مقابل `catalog_category_id` (جديد، مقترَح)
          — لو لم يُضَف العمود الجديد فعلياً قبل تفعيل إنفاذ الهامش، الميزة عديمة الأثر عملياً.
Owner: Founder
Created: 2026-09-19
Review by: قبل أي تكليف Task تنفيذ منفصل يُطبِّق أياً من هذا التصميم على كود/Schema حي (نفس بوابة
          TASK-12 بعد اعتماد PHASE_2_DOMAIN_DESIGN.md، لا اعتماد تلقائي بمجرد وجود هذا السجل)
Blocking: NO — لا يمنع أي عمل قائم اليوم على `dev`/`staging`؛ يمنع فقط بدء تنفيذ هذا التصميم تحديداً
          قبل اعتماد صريح منفصل من المؤسس لكامل الوثيقة
Status: DRAFT — PENDING_FOUNDER_APPROVAL
Related: specs/orders/PHASE_3_SUPPLY_RESOLUTION_DESIGN.md (الوثيقة كاملة — خصوصاً §0 ملخص التحقق الحي،
          §1.1/§1.2 تعارض `ADR-031`، §4.2/§4.3 تصحيح فصل مراجعة المحتوى عن مطابقة الهوية، §4.4/§14
          `catalog_content_review_queue`، §24 سجل القرارات الكامل)، `ADR-031` (النظام القائم الذي
